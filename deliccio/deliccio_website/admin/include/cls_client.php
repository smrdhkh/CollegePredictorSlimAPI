...
<?php
    class clsclient       //class for the tb_client
    {
        ...
        public function Save_Client()      //function for inserting new client in the tb_client
        {
            ...
            $link=$con->db_connect();
            $p=md5($this->pwd);
            $qry="Call insclient('$this->name','$p','$this->email','$this->phone_no')";  //call to stored procedure
        ...
        public function Update_Client()       //function for updating a client in the tb_client
        {
            $con= new clscon();
            $link=$con->db_connect();
            $qry="Call updclient_a($this->id,'$this->name','$this->pwd','$this->email','$this->phone_no')";   //call to stored procedure
            $res=mysqli_query($link,$qry);
            if(mysqli_affected_rows($link))
            {
                $con->db_close();
