...
<?php
    ...
    {
        public $id,$name,$pwd,$email,$address,$phone_no;
        public function Save_Client()      //function for inserting new client in the tb_client
        {
            $con= new clscon();
            $link=$con->db_connect();
            $pass=md5($this->pwd);
            $qry="Call insclient('$this->name','$pass','$this->email','$this->address',$this->phone_no')";  //call to stored procedure
        ...
        public function Update_Client()       //function for updating a client in the tb_client
        {
            ...
            $qry="Call updclient_a($this->id,'$this->name','$this->pwd','$this->email','$this->address','$this->phone_no')";   //call to stored procedure
            $res=mysqli_query($link,$qry);
            if(mysqli_affected_rows($link))
            {
                $con->db_close();
        ...
        public function Login_Client()
        {
            ...
            $link=$con->db_connect();
            $pass=md5($this->pwd);
            $qry="Call loginclient('$this->name','md5',@ret)";
            $res=  mysqli_query($link, $qry);
